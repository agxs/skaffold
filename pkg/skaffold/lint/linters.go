...
func (*RegExpLinter) Lint(lintInputs LintInputs, rules *[]Rule) (*[]Result, error) {
	results := &[]Result{}
...
func (*YamlFieldLinter) Lint(lintInputs LintInputs, rules *[]Rule) (*[]Result, error) {
	results := &[]Result{}
	...
	for _, rule := range *rules {
		...
		if yamlFilter.FieldOnly != "" {
			// TODO(aaron-prindle) this is a bit redundant currently as a filter is run and required still but not used...  Kind of makes edge cases easier, that is all atm
		...
		if node.YNode().Kind == yaml.ScalarNode {
			...
			// TODO(aaron-prindle) does it make sense to continue here? only matter if node.Content() can panic
			// continue
		}
		for _, n := range node.Content() {
			// if rule.RuleID == SkaffoldYamlSyncPython {
			// log.Entry(context.TODO()).Infof("aprindle in loop iteration: %d", i)
			// }
...
func appendRuleIfLintConditionsPass(lintInputs LintInputs, results *[]Result, rule Rule, line, col int) {
	allPassed := true
	// TODO(aaron-prindle) add GenerateExplanation logic here as well?
	explanation := rule.ExplanationTemplate
	if rule.ExplanationPopulator != nil {
		ei, err := rule.ExplanationPopulator(lintInputs)
		if err != nil {
			// TODO(aaron-prindle) log error, possibly also make this return errors
			return
		}
		var b bytes.Buffer
	...
	if allPassed {
		mr := Result{
			...
			Line:        line,
			Column:      col, // column info not accessible via buildkit parse library, use 0 index as a stub when displaying the flagged line
		}
